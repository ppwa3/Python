...
테이블 생성하기
제약조건
	primary key : 기본키 지정. null값이나 중복값을 가질 수 없는 컬럼
		으로 지정됨.
	AUTO_INCREMENT : 자동증가 컬럼으로 지정. 1씩 증가하는 순차적인
		정수값이 자동으로 입력됨. 오라크르이 시퀀스와 유사함.
...
형식1 : 일련번호 idx 컬럼은 insert문에서 생략하고 작성한다.
	...
	부여된다. 즉, 자동증가 컬럼은 insert문에서 생략하는게 기본이다.tb_int
*/
...
DESC tb_strisample_dbsample_dbsample_dbsample_dbsample_dbsample_dbsample_dbng;
...
(
	...
	postdate DATETIME DEFAULT CURRENT_TIMESTAMP, /* 작성일. 현재시각을 디폴트 값으로 지정 */
...
INSERT INTO phonebooks
	(num,username,phonenum,addr)
VALUES ('번호','성명','전화','주소'NOW(),0);
