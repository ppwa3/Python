...
객체를 반환한다. '''
for i, v in enumerate(date1):
  # 리스트를 인덱스와 함께 값 출력
    print(i, v, end=', ')
print()

#딕셔너리 생성
date2 = dict(birth=1970, name="홍길동", size="100cm")
#딕셔너리를 통해 반복하면 Index와 Key를 반환한다.
for i, v in enumerate(date2):
  print(i, v, date2[v], end=', ')
print()


print(f"{'eval()':-^30}")
#실행 가능한 문자열로 입력받아 결과값을 반환한다.
...
while it:
    ...
    if row == 99:
        break
    print(row, end=", ")
''' 위 while문은 더이상 출력할 항목이 없을경우 next()에서 예외가 발생되면서
